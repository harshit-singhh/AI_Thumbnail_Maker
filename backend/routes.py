...
class CreateJobRequest(BaseModel):
    prompt:str = Field(default="There is no prompt")
    num_thumbnails:int
    headshot_url:str
...
@router.get("/jobs/{job_id}" , reponse_model=JobResponse)
def get_job(job_id:str , session: Session = Depends(get_session)):
    ...
    for t in thumbnails:
        variants = get_variants(t.imageKit_url) if t.image_Kit else None
    ...
    return JobResponse(
         id=job.id,
         prompt=job.prompt,
         num_thumbnails=job.num_thumbnail,
